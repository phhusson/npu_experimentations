...
- llama.cpp with Apple NPU prefill -- Bonsai 2 27B prefill upgrades from 60 tok/s to 105tok/s on Apple M4 16GB
- llama.cpp with AMD NPU prefill -- Qwen 3.8-Flash-Next UD_IQ1_S prefill upgrade from to 300tok/s on AMD Ryzen 8845HS
...
For a number of reasons, it is usely not desirable to run token generation on NPU. [1]
...
If you insert NPU work at every layer, you'll make the GPU and CPU loose some precious time.
...
== For a number of reasons, it is usely not desirable to run token generation on NPU.
...
So now you want to make NPU kernel sthat do a lot more ops to reduce synchronization time.
Well shit, not all NPUs are capable of anything. Rockchip RK3588's and Apple's ANE notably can't.

You've made a full NPU kernel that uses 100% of the RAM bandwdith. Fuck yeah, that's awesome!
